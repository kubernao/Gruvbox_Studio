...
export const streamOpenAICompletions: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		const chunkIdleMs = resolveStreamChunkIdleTimeoutMs();
		const streamAbort = new AbortController();
		const requestSignal = options?.signal;
		const activeSignal = requestSignal ?? streamAbort.signal;
		let chunkIdleTimer: ReturnType<typeof setTimeout> | undefined;
		let chunkIdleAbortTriggered = false;
		...
		try {
			...
			const clearChunkIdleTimer = () => {
				if (chunkIdleTimer !== undefined) {
					clearTimeout(chunkIdleTimer);
					chunkIdleTimer = undefined;
				}
			};
			const armChunkIdleTimer = () => {
				clearChunkIdleTimer();
				chunkIdleTimer = setTimeout(() => {
					chunkIdleAbortTriggered = true;
					streamAbort.abort();
				}, chunkIdleMs);
			};

			const { data: openaiStream, response } = await client.chat.completions
				.create(params, { signal: activeSignal })
				.withResponse();
			...
			if (chunkIdleAbortTriggered) {
				output.stopReason = "error";
				output.errorMessage = `Stream idle: no provider chunks for ${Math.round(chunkIdleMs / 1000)}s`;
			...
			if (chunkIdleAbortTriggered && !requestSignal?.aborted) {
				output.stopReason = "error";
				output.errorMessage = `Stream idle: no provider chunks for ${Math.round(chunkIdleMs / 1000)}s`;
			} else {
...
function createClient(
	...
	// Gruvbox API ties Idempotency-Key to a full request hash. Pi sets GRUVBOX_PI_IDEMPOTENCY_KEY
	// once per process; a multi-step run issues several completions with the same key but
	// different bodies → 409 (hash mismatch / in progress). Make each OpenAI client unique.
	if (model.provider === "gruvbox-api") {
		let idem: string | undefined;
		for (const [k, v] of Object.entries(headers)) {
			if (k.toLowerCase() === "idempotency-key" && typeof v === "string" && v.length > 0) {
				idem = v;
				break;
			}
		}
		if (!idem) {
			const fromEnv = process.env["GRUVBOX_PI_IDEMPOTENCY_KEY"];
		...
		if (idem) {
			...
			const suffix = `r${Date.now().toString(36)}${Math.random().toString(16).slice(2, 10)}${Math.random().toString(16).slice(2, 6)}`;
			headers["Idempotency-Key"] = `${idem}-${suffix}`;
		}
	}

	// Gruvbox API rejects duplicate Idempotency-Key while a row is in_progress (503 IDEMPOTENCY_IN_FLIGHT).
	// The OpenAI SDK retries 5xx with the same defaultHeaders → same key → permanent failure. Disable retries.
	...
	if (model.provider === "gruvbox-api") {
		...
		// The SDK defaults to a 10-minute single-request timeout. When the Gruvbox API host is wrong (for example
		// still pointing at localhost while using a deployed API), users wait until that fires. Cap the wait with an
		// env override for legitimately long streamed turns (reasoning models).
		openAiOptions.timeout = resolveGruvboxApiClientTimeoutMs();
	}
	return new OpenAI(openAiOptions);
}

/**
 * Reads `PI_OPENAI_STREAM_CHUNK_IDLE_MS` when set; otherwise 120s. Detects hung SSE streams
 * (connection open but no chunks) before the OpenAI SDK's long default request timeout.
 */
export function resolveStreamChunkIdleTimeoutMs(): number {
	const raw = process.env.PI_OPENAI_STREAM_CHUNK_IDLE_MS;
	if (typeof raw === "string" && raw.trim() !== "") {
		const n = Number.parseInt(raw.trim(), 10);
		if (Number.isFinite(n)) {
			return Math.min(600_000, Math.max(30_000, n));
		}
	}
	return 120_000;
}
...
/**
 * Reads `GRUVBOX_OPENAI_CLIENT_TIMEOUT_MS` when set to a sensible integer; otherwise returns ten minutes so
 * long reasoning and multi-tool agent turns are not cut off by the SDK before Studio's stream idle watchdog.
 * Smaller values are useful when probing unreachable hosts.
 */
function resolveGruvboxApiClientTimeoutMs(): number {
	const raw = process.env.GRUVBOX_OPENAI_CLIENT_TIMEOUT_MS;
	if (typeof raw === "string" && raw.trim() !== "") {
...
function mapStopReason(reason: ChatCompletionChunk.Choice["finish_reason"] | string): {
	...
	switch (reason) {
		...
		// Anthropic Messages / OpenAI shims (e.g. Gruvbox API) surface native stop reasons on finish_reason
		case "end_turn":
		case "pause_turn":
		case "stop_sequence":
			return { stopReason: "stop" };
