...
describe("Coding Agent Tools", () => {
	...
	describe("read tool", () => {
		...
		it("should preserve exact source text for UTF-8 content (no truncation)", async () => {
			const testFile = join(testDir, "utf8-exact.txt");
			const source = "hello\nhéllo\n你好\nemoji 😀\n";
			writeFileSync(testFile, source, "utf-8");

			const result = await readTool.execute("test-call-read-exact-utf8", { path: testFile });
		...
		it("should preserve CRLF source text in read output", async () => {
			const testFile = join(testDir, "crlf-read.txt");
			const source = "line one\r\nline two\r\nline three\r\n";
			writeFileSync(testFile, source, "utf-8");

			const result = await readTool.execute("test-call-read-crlf", { path: testFile });
			const output = getTextOutput(result);

			expect(output).toBe(source);
		});

		it("offset+limit output should match exact source slice", async () => {
			const testFile = join(testDir, "slice-read.txt");
			const lines = Array.from({ length: 20 }, (_, i) => `Line ${i + 1}`);
			writeFileSync(testFile, lines.join("\n"), "utf-8");

			const result = await readTool.execute("test-call-read-slice", { path: testFile, offset: 6, limit: 5 });
			const output = getTextOutput(result);
			const expectedSlice = lines.slice(5, 10).join("\n");

			expect(output.startsWith(expectedSlice)).toBe(true);
			expect(output).toContain("[10 more lines in file. Use offset=11 to continue.]");
		});
