...
describe("RPC prompt response semantics", () => {
	...
	it("emits failure response when preflight succeeds but agent.prompt rejects", async () => {
		const { lineHandler, cleanup } = await startRpcMode({ withAuth: true, responseDelayMs: 0 });
		const promptSpy = vi
			.spyOn(AgentSession.prototype, "prompt")
			.mockRejectedValueOnce(new Error("agent loop failed"));

		try {
			lineHandler(JSON.stringify({ id: "b4", type: "prompt", message: "Hello" }));
